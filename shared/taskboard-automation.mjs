...
export function buildTaskboardAutomationPrompt(request) {
  ...
  const candidateInstructions = [
    "从返回的 todo 中只选择开启了自动化开关（automationEnabled 为 true）且依赖已完成的任务：relations.blockedBy 为空，或其中每个依赖的 status 都严格等于 done。无依赖且开启自动化的 todo 仍可并行处理。未开启自动化开关的任务绝对不得认领或执行。若有 todo 但全部未开启自动化或被未完成依赖阻塞，本轮直接结束，不暂停自动化，也不创建或打开新的任务会话。",
    "每次仅处理一个符合依赖条件的 todo：选定后先用 issue get 读取最新任务内容，并用 comment list 读取全部评论。根据描述和最新评论判断是否允许开始；若其中写明等待、暂不执行或当前不应开始，立即跳过并报告，不改状态。评论也包含已完成后被打回的返工要求。",
    "完成 issue get 和 comment list 后、移动状态前，必须再次运行 issue get，并复核 status 仍为 todo、未归档且 automationEnabled 仍为 true，以及 relations.blockedBy 仍为空或其中每个依赖的 status 都严格等于 done。若依赖条件或自动化开关状态不再满足，立即跳过并结束本轮，不改状态，也不暂停自动化。",
  ];
  const executionInstructions = remoteProject
    ? [
        ...
        "先检查 issue get 的 projectId、version、status、archivedAt、automationEnabled、threadId 和 threadBinding。完整 threadBinding 包含 threadId、codexProjectId、codexProjectKind、codexHostId、workspacePath，且它是该任务后续 send、wait 和状态写回的唯一目标；当前自动化的项目和主机只能作为未绑定任务的首次目标，不能替换已有绑定。若存在 threadId 但没有完整 threadBinding，这是只能由 UI 打开的 legacy local 绑定：使用 comment add 说明自动化无法确认项目和主机，再使用首次读取的 version 作为 --if-version、用 --binding-thread-id 保留原 threadId 将任务移动到 blocked；若冲突立即停止。不得 send、create 或覆盖该绑定。",
        `未绑定任务必须先从上述精确远程项目映射解析 actualTarget。若 developmentContext.type 是 worktree，只保留 codexProjectKind="remote"、codexHostId=${JSON.stringify(request.codexHostId)} 且 workspacePath 与 developmentContext.path 完全相同的项；必须恰好命中一项，并使用该项自己的 codexProjectId、codexHostId 和 workspacePath。零项或多项时使用 comment add 明确记录“目标 SSH worktree 未映射”，随后结束本轮，不认领、不 create、不写基础项目 binding。若没有 worktree，actualTarget 才是上述基础 identity，并且它必须存在于精确映射中。不得回退到基础 root、local、项目名、其他主机或同路径的其他主机。`,
        "确认允许开始后，只有 automationEnabled 为 true、未绑定且仍为未归档 todo 的任务才可在读取代码、下载附件、分析或实施前，由当前本地控制器使用刚读取的 version 移到 in_progress。已有完整 threadBinding 时，issue move 必须同时传 --binding-thread-id、--binding-codex-project-id、--binding-codex-project-kind、--binding-codex-host-id、--binding-workspace-path 的保存值，但在旧会话 send/stale 判断完成前不得把这个 todo 移到 in_progress；stale 清除步骤按后文显式使用 --clear-binding-thread。未绑定时必须传 --clear-binding-thread，避免把本地控制器 CODEX_THREAD_ID 写成任务绑定。写入成功后记录响应 task 的 version 为 ownedVersion、projectId 为 ownedProjectId，并记录本轮 binding；以后本轮每次 issue move 都必须显式传 --if-version ownedVersion，成功后再用响应 version 更新 ownedVersion。不得省略 --if-version 后让 taskctl 自动读取最新 version。写入成功前不得继续。所有认领、评论和状态写入只由当前本地控制器完成，不得要求远程会话运行 taskctl。",
        "若因 version 陈旧发生版本冲突，重新运行 issue get 和 comment list；仅当仍为可认领 todo、绑定身份未变化、未归档且描述和最新评论未变化时，用最新 version 重试一次。若已被认领、绑定、状态或要求已变、已归档、服务或永久 API 错误，或重试仍失败，立即跳过该任务、退出并报告；不得抢占或循环重试。",
        "认领成功后，已有完整 threadBinding 时，只能使用其保存的 threadId 和 codexHostId 调用 Codex send_message_to_thread。send 成功后必须重新 issue get 一次，确认 projectId 未变、未归档、status 仍为 todo 且完整 threadBinding 与保存值完全相同；然后由当前本地控制器使用这次复核返回的最新 version、完整旧 binding 和 --if-version 执行 issue move --status in_progress，传入 --binding-thread-id、--binding-codex-project-id、--binding-codex-project-kind、--binding-codex-host-id、--binding-workspace-path，并记录响应 task.version 为 ownedVersion。认领成功后继续执行后文现有 Codex wait_threads、结果评论和 in_review 写回路径，不得结束本轮；若认领发生 409，立即停止，不得重读新 version 覆盖。只有旧会话工具明确返回终态 NOT_FOUND 或 CLOSED 等会话不存在或已关闭结果时，才确认 stale。timeout、network failure、Codex host 暂时不可达或 Taskboard service unavailable 都不是 stale：保留 binding 并结束本轮，不得猜测、clear、create 或抢占。若任务已是 in_progress、活跃、已归档、状态或 binding 已变化，立即停止，不得在当前自动化目标创建替代会话。只有未绑定任务才使用 Codex create_thread 创建远程任务，target 必须是 {type:\"project\",projectId:actualTarget.codexProjectId,environment:{type:\"local\"}}，首次 identity 必须使用 actualTarget 的 projectId、kind=\"remote\"、hostId 和 workspacePath。发送给远程会话的指令必须包含任务编号、标题、完整描述、全部评论和开发上下文，并说明远程会话不运行 taskctl，只需完成实现、验证并返回改动、结果和剩余风险。",
        "确认旧会话 stale 后，必须先用 comment add 保存一条历史记录，并同时传 --thread-id、--binding-thread-id、--binding-codex-project-id、--binding-codex-project-kind、--binding-codex-host-id 和 --binding-workspace-path 的完整旧 binding；评论写入成功后，再使用同一次 issue get 的 version 执行 issue move --status todo --clear-binding-thread --if-version。评论或清除失败立即停止，不得认领。然后只重新 issue get 一次；仅当 projectId 未变、未归档、status 仍为 todo、threadId 为空且 threadBinding 为空时，才进入未绑定任务的现有认领和 create_thread 路径。",
        "仅当 send_message_to_thread 成功，或 create_thread 成功返回远程 threadId，才视为远程 worker 已确认。未绑定任务在 create_thread 失败时，使用 comment add 记录失败工具和错误；随后用 ownedVersion、显式 --if-version 和 --clear-binding-thread 将当前任务移回 todo 并结束。若发生 409，说明其他控制端已修改任务，立即停止且不得重读最新 version 后覆盖。此补偿只处理本轮当前已认领任务；不得扫描或接管其他 in_progress。",
        "新建远程任务成功后，使用 ownedVersion 和显式 --if-version 再次移动到 in_progress；必须用完整 binding 参数保存 create_thread 返回的 threadId，以及 actualTarget 的 projectId、kind=\"remote\"、hostId 和 workspacePath。成功后用响应 version 更新 ownedVersion 和本轮 binding。若请求响应丢失或结果不确定，只允许重新 issue get 一次；仅当 projectId 等于 ownedProjectId、未归档、状态仍为本轮 in_progress，且 threadBinding 为空或与本轮五字段 binding 完全相同时才可继续。读到相同 binding 视为前次保存成功；读到空 binding 时才可用本次核对后的 version 重试一次；读到不同 binding 或任一其他核对项变化时立即退出，不得写回。若确定绑定写入失败，使用 comment add 记录失败和远程 threadId，再用 ownedVersion、显式 --if-version 和同一完整 binding 将任务移动到 blocked；409 时停止且不得重复派发。",
        "使用 Codex wait_threads 等待远程会话时，目标必须使用任务保存的 threadBinding.threadId 和 threadBinding.codexHostId。wait_threads 失败、远程会话明确需要用户输入或无法继续时，使用 comment add 记录原因，再用 ownedVersion、显式 --if-version 和完整保存 binding 将任务移动到 blocked；409 时立即停止。远程会话完成后，使用 comment add 写入改动、验证结果、执行结果和剩余风险，再用 ownedVersion、显式 --if-version 和完整保存 binding 将任务移动到 in_review。worker 确认后的每一次 issue move 都必须显式传完整远程 binding；不要把未完成工作标记为 in_review。",
      ]
    : [
        ...candidateInstructions,
        `本定时任务由 Codex 在独立 worktree 中运行。确认允许开始后，只有 automationEnabled 为 true、threadId 和 threadBinding 都为空且仍为未归档 todo 的任务才可认领。认领前按最新 issue.developmentContext 选择实际开发目录，不能未经选择就把当前自动化 worktree 当作任务目录。若 type="branch"，使用该已存在分支：通过 git worktree list --porcelain 找到检出它的工作树；若尚未检出，则先创建任务工作树路径的父目录，再用 git worktree add <任务工作树路径> <指定分支> 检出该分支。不得将指定分支重置到默认分支。若 type="worktree"，沿用其 path 和 branch。需要新建工作树时，路径取仓库根目录同级的 <仓库名>-taskboard-worktrees/<任务编号>，任务编号使用 issue.identifier。`,
        `只有 developmentContext 为空时，才从项目基础目录 ${JSON.stringify(request.workspacePath)} 运行 git rev-parse HEAD，使用这个本地提交作为基线；以 codex/automation/<任务编号> 为本地新分支，先创建任务工作树路径的父目录，再执行 git worktree add -b <新分支> <任务工作树路径> <本地基线提交>。准备工作树时不 fetch、不 push，也不在远端创建分支。同编号工作树已存在时，只能在路径和分支一致时复用；不得重置或覆盖其他工作树。认领前核对实际开发目录的绝对路径和所属仓库；type="branch" 时当前分支必须等于 developmentContext.branch，type="worktree" 时核对其已记录的 branch（若非空），developmentContext 为空时当前分支必须等于新分支。任一步失败时用 comment add 记录原因并结束。`,
        `准备目录后再运行 issue get，确认 developmentContext 与准备时完全相同且任务仍可认领。认领必须显式传这次读取的 --if-version，通过 issue update 一次写入 --status in_progress：原开发上下文为 branch 或 worktree 时保留它，不传新的开发上下文参数；原开发上下文为空时传 --worktree-path 为任务工作树的绝对路径、--worktree-branch 为新分支。无论哪种情况，都显式传 --binding-thread-id "$CODEX_THREAD_ID"、--binding-codex-project-id ${JSON.stringify(request.codexProjectId)}、--binding-codex-project-kind "local"、--binding-codex-host-id ${JSON.stringify(request.codexHostId)}、--binding-workspace-path 为实际开发目录的绝对路径，使完整 binding 和状态指向真正执行的目录；记录响应 task.version 为 ownedVersion。写入成功前不得读取代码、下载附件、分析或实施。已有完整 binding 或 legacy local binding 的任务必须先按旧会话规则处理；不得认领已被其他会话绑定或其他 Agent 领取的任务。认领后的每一次 issue move 都必须显式传 ownedVersion 和这五个完整 binding 字段，成功后更新 ownedVersion。`,
        "若因 version 陈旧发生版本冲突，重新运行 issue get 和 comment list；仅当仍为可认领 todo、未绑定其他会话、未归档且 description、developmentContext 和最新评论均未变化时，用最新 version 重试一次。若已被认领、状态或要求已变、已归档、服务或永久 API 错误，或重试仍失败，立即跳过该任务、退出并报告；不得抢占或循环重试。",
        ...
        "若任务已绑定原会话的 branch 或 worktree，必须继续使用该绑定；新认领任务的实现和直接验证只能在认领时选定的实际开发目录中完成，所有文件和 Git 操作都明确指定该目录。不得把后续任务的改动加入此目录。",
        "执行完成并验证后，先用 comment add 记录关键改动、验证结果、执行结果和剩余风险，并附上实际开发目录，再使用 ownedVersion、显式 --if-version 和认领时保存的完整 binding 将任务移动到 in_review；成功后更新 ownedVersion。保留该目录、分支、developmentContext 和 binding 供评审使用，不得清理或复用给下一项任务；不要省略 binding，避免把完整绑定降级为 legacy local；不要直接标记为 done。",
      ];
  return [
